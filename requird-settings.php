...
class Requird {
	...
	var $default_options = array(
		...
		'error_fields' => 'Fill in the required fields'
	);
	...
	function upgrade_options($options) {
		...
		if (!isset($options['error_message'])) {
			$options['error_message'] = $this->default_options['error_message'];
			$options_need_updating = true;
		}

		if (!isset($options['error_fields'])) {
			$options['error_fields'] = $this->default_options['error_fields'];
			$options_need_updating = true;
		}
	...
	function text_field_callback($args) {
		$html = '<input type="text" id="' . $args['slug'] . '" name="requird[' . $args['slug'] . ']" value="' . $this->options[$args['slug']] . '" style="width:100%"/>';
		$html .= '<label for="' . $args['slug'] . '" style="display: block;"> ' . $args['description'] . '</label>';
		echo $html;
	}
