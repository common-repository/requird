...
function requirdformSubmit() {
	var $ = jQuery,
		...
		requird = {
			...
			message: '<div class="requird-fields" style="color:red; font-weight: bold; font-size: 16px; margin:10px 0;">' + requirdOptions.error_message + '</div>',
			publishing: '<div class="requird-publishing" style="color:red; font-size: 14px; margin:10px;"><i class="dashicons dashicons-info" style="vertical-align: text-top;"></i> ' + requirdOptions.error_fields + '</div>',
			errors: 0
		};
	...
	if (requird.errors) {
		...
		$('.requird-publishing').remove();
		$('#submitdiv').append(requird.publishing);
		return false;
	} else {
		$('.requird-publishing').remove();
	}
...
jQuery(document).ready(function($) {
	$('form#post').on('submit', function(e) {
		if ($('#post_status').val() === 'publish' || $("#publish:focus").size()) {
